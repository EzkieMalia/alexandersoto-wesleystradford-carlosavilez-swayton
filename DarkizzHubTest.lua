...
FovCircle.Visible = FovVisibleEnabled
FovCircle.Radius = FovCircleSize
FovCircle.Thickness = 1.5
FovCircle.Transparency = 1
FovCircle.Color = Color3.new(1,1,1)
FovCircle.Filled = false
FovCircle.Position = Vector2.new(Mouse.X, Mouse.Y + 57.5)
end)
...
RunService.RenderStepped:Connect(function()
    ...
    local Nearest_Head = nil
    if Aiming and FovEnabled and SilentAim then
        for i,v in next, Players:GetChildren() do
            if v ~= LocalPlayer and v.Character and v.Character:FindFirstChild("HumanoidRootPart") and v.Character:FindFirstChild("Humanoid") and v.Character:FindFirstChild("Humanoid").Health > 0 then
                if v.Team == LocalPlayer.Team then
                    local Character = v.Character
                    local HRP, IsOnScreen = CurrentCamera:WorldToViewportPoint(Character:FindFirstChild("HumanoidRootPart").Position)
                    if IsOnScreen then
                        local Magnitude = (Vector2.new(Mouse.X, Mouse.Y) - Vector2.new(HRP.X, HRP.Y)).Magnitude
                        if Magnitude < Distance and Magnitude < FovCircle.Radius then
                            Distance = Magnitude
                            Nearest_Head = Character
                        end
                    end
                end
            end
            if Nearest_Head ~= nil and Nearest_Head:FindFirstChild("HumanoidRootPart") and Nearest_Head:FindFirstChild("Humanoid") and Nearest_Head:FindFirstChild("Humanoid").Health > 0 then
                CurrentCamera.CFrame = CFrame.new(CurrentCamera.CFrame.Position, Nearest_Head:FindFirstChild("HumanoidRootPart").Position)
            end
        end
    end
end)
...
local function MainEsp(target)
    ...
    healthtext.Position = Vector2.new(minX - (maxX - minX)/5, minY - healthtext.TextBounds.Y - 4) 
...
local Section9 = HitboxTab:CreateSection("Head Size (POSSIBLY DETECTED! ⚠️)")
...
local Section11 = HitboxTab:CreateSection("Aim Lock (POSSIBLY DETECTED ⚠️)")
...
local Toggle7 = HitboxTab:CreateToggle({
   Name = "FOV",
   CurrentValue = false,
   Flag = "Toggle7",
   Callback = function(Value)
...
local Toggle8 = HitboxTab:CreateToggle({
   Name = "Draw FOV",
   CurrentValue = false,
   Flag = "Toggle8",
   Callback = function(Value)
    FovVisibleEnabled = Value
   end,
})

local Slider11 = HitboxTab:CreateSlider({
   Name = "Fov Radius",
   Range = {25, 250},
   Increment = 5,
   Suffix = "Radius",
...
local Divider11 = HitboxTab:CreateDivider()
